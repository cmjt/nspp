...
#' @export
sim.ns <- function(pars = NULL, lims = rbind(c(0, 1), c(0, 1)), rchild = rpois, dispersion="gaussian", non.siblings = NULL, plot.points = FALSE, plot.empirical = FALSE){
    ...
    if (plot.points){
        ...
        analytic.palm(Dc, nu, child.disp, n.dims, c(0, 1),dispersion=dispersion,add = TRUE)
    }
    child.locs
}
...
#' are \code{D}, whale density, \code{child.disp}, whale movement,
...
sim.twoplane <- function(pars, lims){
    ...
    movement <- sample(c(-1, 1), size = n.whales, replace = TRUE)*
        child.disp*sqrt(2)*sqrt(rchisq(n.whales, 1))
